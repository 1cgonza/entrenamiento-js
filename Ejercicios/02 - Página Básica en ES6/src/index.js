...
import { colorAleatorio } from './ayudas';
...
let timer_is_on = true; //se declara la variable constante timer_is_on

function cambioDeColor() {
  //se crea la variable constante intervalo, que contiene la función que permite el funcionamiento del contador
  numero.innerText = contador; //el contador se pone como texto interno del div number

  if (contador > 0 && contador % 5 === 0) {
    //se busca el múltiplo de 5 sin contar el 0
...
function iniciarIntervaloDeColor() {
  intervalo = setInterval(cambioDeColor, 1000); // milisegusgos del intervalo
}

function botonColor() {
  //función que se activa al hacer click en el botón que cambia el color de main
  fondo.style.backgroundColor = colorAleatorio(); //cambia el color de main a partir de la función colorAleatorio
}

function botonPausa() {
  //función que se activa al hacer click al botón de pausa
  clearTimeout(intervalo); //suspende el intervalo de tiempo del contador
  timer_is_on = false;
}

function botonInicio() {
  //función que se activa al hacer click al botón de inicio
  if (!timer_is_on) {
    timer_is_on = true;
    iniciarIntervaloDeColor();
  }
}

iniciarIntervaloDeColor();

document.getElementById('pausaInput').addEventListener('click', botonPausa); //se declara la variable constante pausa que activa la función botonPausa al hacer click sobre el botón
document.getElementById('inicioInput').addEventListener('click', botonInicio); //se declara la variable constante inicio que activa la función botonInicio al hacer click sobre el botón
document.getElementById('colorInput').addEventListener('click', botonColor); //se declara la variable constante color que activa la función botonColor al hacer click sobre el botón

// function colorAleatorio() {
//   //función a partir de la cual se cambia aleatoriamente el color de fondo de main
